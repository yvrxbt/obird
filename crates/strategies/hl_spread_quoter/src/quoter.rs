//! HlSpreadQuoter — two-level symmetric spread market maker.
//!
...
impl State {
    ...
    fn cooldown_expired(&self) -> bool {
        matches!(self, State::Cooldown(t) if Instant::now() >= *t)
    }
}
...
impl HlSpreadQuoter {
    ...
    fn bid_price(&self, mid: Decimal, level: usize) -> Decimal {
        mid * (Decimal::ONE - self.params.level_ratio(level))
    }

    fn ask_price(&self, mid: Decimal, level: usize) -> Decimal {
        mid * (Decimal::ONE + self.params.level_ratio(level))
    }

    /// Drift = max across all levels of how far new target prices are from resting prices.
    /// This is order-price-based drift: "how stale are my current quotes?"
    /// Works correctly even with position skew (resting prices may differ from pure-mid offsets).
    fn max_drift_bps(&self, mid: Decimal) -> Decimal {
        ...
        for (level, &(resting_bid, resting_ask)) in self.resting_prices.iter().enumerate() {
            if resting_bid.is_zero() && resting_ask.is_zero() {
                continue; // no resting order at this level
            }
    ...
    fn build_place_actions(&self, mid: Decimal) -> Vec<Action> {
        let mut orders = vec![];

        for level in 0..self.params.level_bps.len() {
            let bid = self.bid_price(mid, level);
            let ask = self.ask_price(mid, level);
            ...
            if self.net_position < self.params.max_position {
                orders.push(Action::PlaceOrder(OrderRequest {
                    ...
                    tif: TimeInForce::PostOnly, // ALO — always maker
    ...
    fn requote(&mut self, mid: Decimal) -> Vec<Action> {
        ...
        let place_actions = self.build_place_actions(mid);

        // Store the prices we're actually placing at so drift can compare against them.
        // Compute outside the mutable borrow to satisfy the borrow checker.
        let new_resting: Vec<(Decimal, Decimal)> = (0..self.params.level_bps.len())
            .map(|l| (self.bid_price(mid, l), self.ask_price(mid, l)))
            .collect();
...
impl Strategy for HlSpreadQuoter {
    ...
    async fn on_event(&mut self, event: &Event) -> Vec<Action> {
        match event {
            ...
            Event::BookUpdate { book, .. } => {
                ...
                // Log every mid at debug — goes to log file only, not terminal.
                // This gives a full price trace so we can always reconstruct what happened.
                ...
                match self.state {
                    ...
                    State::Cooldown(_) => return vec![], // still waiting
                    _ => {}
                }

                // Drift check: compare where quotes WOULD be now vs where they ARE.
                // Using order-price-based drift so skewed quotes are measured correctly.
                if self.state.is_quoting() {
                    let drift = drift_now; // already computed above
                    if drift > Decimal::from(self.params.drift_bps) {
                        tracing::warn!(
                            target: "quoter",
                            strategy = %self.id,
                            mid = %mid,
            ...
            // OrderUpdate: we don't track individual order state anymore,
            // but log rejections for visibility.
    ...
    async fn initialize(&mut self, state: &StrategyState) -> Vec<Action> {
        ...
        // Cancel any orphan orders from a previous run before placing new ones
        vec![Action::CancelAll { instrument: self.instrument.clone() }]
    }

    async fn shutdown(&mut self) -> Vec<Action> {
        tracing::info!(target: "quoter", strategy = %self.id, "SHUTDOWN");
        vec![Action::CancelAll { instrument: self.instrument.clone() }]
    }
}
